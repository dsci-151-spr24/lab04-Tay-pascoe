 ...
 ***There are `r nrow (dennys)` rows and `r ncol (dennys)` columns. Each row represents data from one Denny's restaruant. The variables are: `r names(dennys)`.***

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
 ***There are `r nrow (laquinta)` rows and `r ncol (laquinta)` columns. Each row represents data from one La Quinta Inn restaruant. The variables are: `r names(laquinta)`***
 ...
 ***La Quintas are mostly loated in the Americas but do have locations in other parts of the world. Denny's is a chain restaurant only in the U.S.***
...
***you can look to see where their locations are in the dataset to see if they are in U.S states or not.***
...
```
***All denny's locations are in the US***
...
laquinta %>% 
  ...
  arrange (desc(n))
```
***California and Texas have the most Denny's and Vermont and Deleware have the lease. Texas and Florida have the most Laquintas and South Dakota and Vermont are of the lowest. This is not all that surprising for Denny's since those are the biggest states. For Laquinta, Texas makes since for being the most but Florida was suprsinging because it's further away from the border and it seems too be a more spanish hotel chain.***
...
***For dennys, the distract of Columbia and Rhode Island have the most locations per thousand square miles. For Laquinta, Rhode Isaland and Florida have the most per thousand square mile. ***
...
***there seems to be more denny's scattered without La Quinta, so no. ***
...
12. Now filter the data for observations in Texas only, and recreate the plot, with an appropriate `alpha` level.
  ...
  ***This plot shows his joke to be a little more true with there being a fair amount of Denny's and La Quintas near each other.***
